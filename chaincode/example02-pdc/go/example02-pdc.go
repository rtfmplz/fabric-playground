...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response {
	...
	// err = stub.PutPrivateData(pdcName, A, []byte(strconv.Itoa(Aval)))
	// if err != nil {
	// 	return shim.Error(err.Error())
	// }

	// err = stub.PutPrivateData(pdcName, B, []byte(strconv.Itoa(Bval)))
	// if err != nil {
	// 	return shim.Error(err.Error())
	// }
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "invoke" {
		...
		// the old "Query" is now implemtned in invoke
		stub.SetEvent("Invoke5", []byte("payload"))
...
func (t *SimpleChaincode) invoke(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	Bvalbytes, err := stub.GetPrivateData(pdcName, B)
	if err != nil {
		return shim.Error("Failed to get state")
	}
	if Bvalbytes == nil {
...
func (t *SimpleChaincode) set(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if len(args) != 2 {
		return shim.Error("Incorrect number of arguments. Expecting name of the person to set")
	}

	stub.SetEvent("set", []byte("payload"))
	A = args[0]
